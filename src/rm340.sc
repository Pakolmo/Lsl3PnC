...
;;;						{Just say what you want to say:} ;English
;;;						#title {Author Interface} ;English
						{Simplemente di lo que quieras:} ;Spanish
						#title {Interfaz del Autor} ;Spanish
					)
				)
		...
		(cond 
			...
			(
				...
				(cond 
					...
					(else
						(HandsOff) ;add for text exit
						(self changeState: 4)
						(theGame setCursor: 999) ;ADD ;add for text exit
					)
				)
			)
			((Said 'lie')
	...
	(		
			(and
				...
				(if ;check if clicked on Al

						(ClickedOnObj aAl (event x?) (event y?))
					...
					(switch theCursor
						...
						(if
							(or
								(& (ego onControl:) cCYAN)
								(& (ego onControl:) cGREEN)
								(& (ego onControl:) cWHITE)
								(& (ego onControl:) cBLACK)
							)
								(if (Btst fAlAndBill)
									(Print 340 11)
								else
				...
				(if ;check if clicked on Al_TOP		(DrawRect 97 123 146 175 8)

				(and ;AL TOP
						(> (event x?) 95) ;x1 (> (mouseX) (left edge of rectangle))
						(< (event x?) 123) ;x2 
						(> (event y?) 146) ;y1
						(< (event y?) 175) ;y2
						(cast contains: aAl)
				...
				(if ;check if clicked on Bill

						(ClickedOnObj aBill (event x?) (event y?))
			...
			(if
				(and ;exit room
						(> (event x?) 139) ;x1 (> (mouseX) (left edge of rectangle))
						(< (event x?) 181) ;x2 
						(> (event y?) 181) ;y1
						(< (event y?) 189) ;y2
					)
				...
				(if ;check if clicked on Bottle wine

						(ClickedOnObj aBottle (event x?) (event y?))
			...
			(if
				(and ;seat
						(> (event x?) 155) ;x1 (> (mouseX) (left edge of rectangle))
						(< (event x?) 178) ;x2 
						(> (event y?) 122) ;y1
						(< (event y?) 141) ;y2
					)
...
;;;					(GetInput @ethnic1Str 15 {Ethnic group 1:}) ;english
					(GetInput @ethnic1Str 15 {Grupo etnico 1:}) ;spanish
				)
				(while (> 3 (StrLen @ethnic2Str))
;;;					(GetInput @ethnic2Str 15 {Ethnic group 2:}) ;english
					(GetInput @ethnic2Str 15 {Grupo etnico 2:}) ;spanish
				)
				(while (> 3 (StrLen @ethnic3Str))
;;;					(GetInput @ethnic3Str 15 {Ethnic group 3:}) ;english
					(GetInput @ethnic3Str 15 {Grupo etnico 3:}) ;spanish
				)
				(Print 340 48)
				(= seconds 3)
			)
							...
							;(if (< temp205 (StrLen @str))
								...
								(if (== 47 (StrAt @str temp205))
									...
									(switch (StrAt @str (++ temp205))
										...
										(else ;[len 0] should never be empty.
											(= [len i] @ethnic3Str)
											(++ i)
										)
									)
			...
			(24
				...
				(RoomScript changeState: 4); Larry levantarse show.
				(theGame setCursor: 999)	
			)
				)
