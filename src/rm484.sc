...
(instance rm484 of Room
	...
	(method (init)
		...
		(if (== prevRoomNum 470)
			...
			(Format ((Inventory at: iMoney) name?) {47 d/lares en propinas.}) ;Spanish
;;;			(Format ((Inventory at: iMoney) name?) {47 dolars on tips._____}) ;English
...
(instance RoomScript of Script
	...
	(method (handleEvent event)
		...
		(cond 
			...
			(if
					(and ;Dress (DrawRect 1 57 172 189 7)
						...
						(if
				(and ;exit room
						(> (event x?) 138) ;x1 (> (mouseX) (left edge of rectangle))
						(< (event x?) 180) ;x2 
						(> (event y?) 180) ;y1
						(< (event y?) 189) ;y2
					)
					...
					(if ;check if clicked on aBra
					(and
						(ClickedOnObj aBra (event x?) (event y?))
						(cast contains: aBra)
					)
						(event claimed: TRUE)
						(switch theCursor	
						...
						(cond 
							...
							(if ;check if clicked on aPanties
					(and
						(ClickedOnObj aPanties (event x?) (event y?))
						...
						(995 ; GET
							...
							(else
								(event claimed: FALSE)
									...
									(if ;check if clicked on aPantyhose
					(and
						(ClickedOnObj aPantyhose (event x?) (event y?))
				...
				(if
					...
					(and ;bed
						...
						(> (event x?) 114) ;x1 (> (mouseX) (left edge of rectangle))
						(< (event x?) 216) ;x2 
						(> (event y?) 78) ;y1
						(< (event y?) 105) ;y2
					)
					...
					(and ;piano
						...
						(> (event x?) 271) ;x1 (> (mouseX) (left edge of rectangle))
						(< (event x?) 319) ;x2 
						(> (event y?) 100) ;y1
						(< (event y?) 154) ;y2
					)
				...
				(if
					(and
						(ClickedOnObj aWine (event x?) (event y?))
						(cast contains: aWine)
					)
