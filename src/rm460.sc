...
;;;								{A clerk stands bored behind the counter.} ;english
								{Un empleado se aburre tras el mostrador.} ;spanish
							)
						)
					)
				)
				...
				(if ;check if clicked on aButton
					(ClickedOnObj aButton (event x?) (event y?))
					(event claimed: TRUE)
					(switch theCursor
						(998 ;look
          						(Printf 460 3
				...
				(if ;check if clicked on aMan
					(and
					(ClickedOnObj aMan (event x?) (event y?))
					(cast contains: aMan)
					)
					(event claimed: TRUE)
					(switch theCursor
