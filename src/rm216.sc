...
(procedure (KalalauSays &tmp seconds)
	(Print @str
		#at 10 15
;;;		#title {Kalalau says...} ;ENGLISH
		#title {Kalalau dice...} ;SPANISH
		#width 160
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(10
				...
				(Print
					(Format @str 216 41
;;;						(if (>= filthLevel 3) { lesbian} else {}) ;ENGLISH
						(if (>= filthLevel 3) { lesbiana} else {}) ;SPANISH
					)
					#dispose
					#time 28
				)
	...
	(method (handleEvent event)
		...
		((if
			...
			(if (ClickedOnObj aMailBox (event x?) (event y?))
				(event claimed: TRUE)
				(switch theCursor
					(995 ;Take
						(cond 
							...
							((not (& (ego onControl:) cBLUE))
								(ego setMotion: MoveTo  130 126)
								(self changeState: 12)		
							)
							(mailboxOpen
								(Print {Ya est* abierto}) ;spanish
;;;								(Print {It's opened}) ;english
							)
							(else
								(self changeState: 12)
							)
					...
					(else 
						(event claimed: FALSE) ;allow the click to be handled by other scripts
					)
				)	
			)	
				...
				(if
					(and ;fence
						...
						(> (event x?) 175) ;x1 (> (mouseX) (left edge of rectangle))
						(< (event x?) 319) ;x2 
						(> (event y?) 69) ;y1
						(< (event y?) 143) ;y2
					)
						(event claimed: TRUE) ;inital claimed
				(switch theCursor
					...
					(else ;we didn't need it so...
						(event claimed: FALSE)	;release the claim!
					)
				)
				)
