...
(instance RoomScript of Script
	...
	;	(if (or (!= (event type?) saidEvent) (event claimed?))
		...
		(cond 
					...
					(switch theCursor
						...
						(else
							(event claimed: FALSE)
											...
											(if
					(and ;background (DrawRect 57 249 122 181 1)
						
						(> (event x?) 57) ;x1 (> (mouseX) (left edge of rectangle))
						(< (event x?) 249) ;x2 
						(> (event y?) 122) ;y1
						(< (event y?) 181) ;y2
					)
			...
			(if
						(ClickedOnObj aRobin (event x?) (event y?))
...
;
		...
		;;;											#button {Door} 1 ;english
		;;;											#button {Key card} 2 ;english
		;;;											#button {Bambi Tape} 3 ;english
		;;;											#button {Locker} 4 ;english
		;;;											#button {Combination} 5 ;english
											
													{&Qu+ quieres preguntar?}
													#button {Hablar} 1
													#button {Puerta} 2
													#button {Tarjeta} 3 
;;;													#button {Cinta de Bambi} 4
													#button {Taquilla} 5 
													#button {Combinaci>n} 6
								)
							)
										(switch temp0	
											(1 ; hablar
...
;;;							(if ;check if clicked on aStudioDoor, locker and other door.
;;;						(or
;;;						(ClickedOnObj aStudioDoor (event x?) (event y?))
;;;						(ClickedOnObj aLockerDoor (event x?) (event y?))
;;;						(ClickedOnObj aTanBoothDoor (event x?) (event y?))
;;;						)
;;;					(event claimed: TRUE)
;;;
;;;					
...
;;;					((not (ego has: iSoap)) ;fixed in room300
;;;						(Print 360 7)
;;;					)
					(
						(and
			...
			(else
				(event claimed: FALSE)
						...
						(ClickedOnObj atpRearCardHole (event x?) (event y?))
			...
			(else
				(event claimed: FALSE)
						...
						(ClickedOnObj atpLeftCardHole (event x?) (event y?))
			...
			(else
				(event claimed: FALSE)
						...
						(ClickedOnObj atpRightCardHole (event x?) (event y?))
...
;;;			(if ;check if clicked on atpRightCardHole, atpLeftCardHole and atpRearCardHole.
;;;				(or
;;;						(ClickedOnObj atpRearCardHole (event x?) (event y?))
;;;						(ClickedOnObj atpLeftCardHole (event x?) (event y?))
;;;						(ClickedOnObj atpRightCardHole (event x?) (event y?))
...
;;;		
;;;									
				(if ;check if clicked on Board

						(ClickedOnObj atpBboard (event x?) (event y?))
