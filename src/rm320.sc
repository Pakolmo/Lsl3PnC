...
(instance RoomScript of Script
	...
	(method (handleEvent event)
		...
		(cond 
			...
			(if
					(and ;background  9 301 21 163 1
						(> (event x?) 9) ;x1 (> (mouseX) (left edge of rectangle))
						(< (event x?) 301) ;x2 
						(> (event y?) 21) ;y1
						(< (event y?) 163) ;y1
					)
			...
			(if
					(and ;plant2  9 301 21 163 1
						(> (event x?) 277) ;x1 (> (mouseX) (left edge of rectangle))
						(< (event x?) 313) ;x2 
						(> (event y?) 117) ;y1
						(< (event y?) 180) ;y1
					)
				...
				(if
					...
					(and
						...
						(cast contains: aRoger);  ;checks that is on the screen
					)
					(event claimed: TRUE)
...
;;;											#button {decree} 1 ;English
;;;											#button {land} 2 ;English
											{&De qu+ quieres hablar?} ;Spanish
											#button {Divorcio} 1 ;Spanish
											#button {Tierras} 2 ;Spanish								
...
;;;															{business doing pleasure} ;English
															{Los negocios dan placer} ;Spanish
														else
;;;															{pleasure doing business} ;English
															{El placer da negocios} ;Spanish
														)
													)
													(= lawyerState LSdone)
	...
	(method (changeState newState)
		...
		(switch (= state newState)
			...
			(3
				...
				(Print @str
					#at -1 10
;;;					#title {Roger says} ;English
					#title {Roger dice} ;Spanish
					#mode teJustCenter
					#icon 321 5 0
				)
				(User canInput: TRUE)
