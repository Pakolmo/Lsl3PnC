		...
		(cond 
			...
			(
				...
				(if
					...
					(and ;sand to put towel 
						
						(> (event x?) 35) ;x1 (> (mouseX) (left edge of rectangle))
						(< (event x?) 140) ;x2 
						(> (event y?) 129) ;y1
						(< (event y?) 172) ;y2
					)
				...
				(if
					...
					(and
						(ClickedOnObj aTawni (event x?) (event y?))
						(cast contains: aTawni);  ;checks that is on the screen
					)
					(event claimed: TRUE)
					(switch theCursor					
						(998 ; Look
			 ...
			 (else
				...
				(if ;check if clicked on towel

						(ClickedOnObj aTowel (event x?) (event y?))	
						(event claimed: TRUE)
						(switch theCursor					
							(998 ;look
							)
							(995 ; Use
								(cond 
											...
											((not (ego has: iTowel))
												;(DontHave) ;skip "No lo tienes" text.
											)
											(else
												(Ok)
												(self changeState: 13)
