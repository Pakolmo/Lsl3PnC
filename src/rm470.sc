		...
		(cond 
			...
			((Said 'look>')
				(cond 
					...
					((Said '/door')
;;;						(Printf 470 14 (if (aDoor cel?) {open} else {closed})) ;english
						(Printf 470 14 (if (aDoor cel?) {abierta} else {cerrada})) ;spanish
					)
					((Said '/(hole<key),bolt')
						(Print 470 15)
					)
				...
				(not (& (event modifiers?) emRIGHT_BUTTON))
						...
						(if ;check if clicked on atpPanel
			...
			;;;						(Printf 470 14 (if (aDoor cel?) {open} else {closed})) ;english
					...
					(if ;check if clicked on atpPanel
						(ClickedOnObj atpPanel (event x?) (event y?))
					...
					(30 ;penthousekey patti
						(event claimed: 1)
						
						(= temp1
										(Print
;;;											{Where do you want to go?} ;english
;;;											#button {Penthouse} 1 ;english
;;;											#button {Down} 2 ;english

											
											{&A d>nde quieres ir?}
											#button {*tico} 1 ;Spanish
											#button {abajo} 2 ;Spanish
