	...
	(method (handleEvent event)
		...
		(cond 
			...
			(if
				...
				(if (ClickedOnObj ego (event x?) (event y?))  ;clicked Larry skirt
					(if (== theCursor 994)
						(event claimed: TRUE)
						(cond 
					((& (ego onControl:) cBROWN)
			...
			(if ;girl
					(ClickedOnObj aJodi (event x?) (event y?))
					(event claimed: TRUE)
					(switch theCursor					
						(998 ; look girl
							(cond 
				...
				(if ;check if clicked on Al Lowe
					(or
						(ClickedOnObj alsHead (event x?) (event y?))
						(ClickedOnObj alsFeet (event x?) (event y?))
					)
					(event claimed: TRUE)
			...
			(if
				(and ;barrel
						(> (event x?) 236) ;x1 (> (mouseX) (left edge of rectangle))
						(< (event x?) 251) ;x2 
						(> (event y?) 110) ;y1
						(< (event y?) 128) ;y2
					)
			...
			(if
				(and ;fountain
						(> (event x?) 48) ;x1 (> (mouseX) (left edge of rectangle))
						(< (event x?) 64) ;x2 
						(> (event y?) 104) ;y1
						(< (event y?) 134) ;y2
					)
