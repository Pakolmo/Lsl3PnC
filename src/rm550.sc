...
(instance rm550 of Room
	...
	(method (init)
		...
		(Load SOUND 6)
		(super init:)
		(User canInput: FALSE canControl: FALSE mapKeyToDir: FALSE) ;add quitar teclado
...
;;;		(if (or (!= (event type?) saidEvent) (event claimed?))
		...
		(cond 
			((Said 'bang,board,(climb,lie,get<on)/log') ;subir a log.
				(cond 
					((!= currentStatus egoNORMAL)
						(GoodIdea)
					)
			...
			((Said 'look>')
				...
				(if
					...
					(if ;check if clicked on log
						(ClickedOnObj aLog (event x?) (event y?))
...
;;;								#button {Move Log} 1 ;english
;;;								#button {Get on Log} 2 ;english

												
								{&Qu+ quieres hacer?}
								#button {Mover Tronco} 1 ;Spanish
								#button {Subir al Tronco} 2 ;Spanish
										)
						)
							(switch temp3
							(1 ;Mover
					...
					(else
						(self changeState: 4)
						(User canInput: FALSE canControl: FALSE mapKeyToDir: FALSE)
					)
				)
							)(else
					(event claimed: FALSE)
