	...
	(method (changeState newState)
		...
		(switch (= state newState)
			...
			(9
				...
				(User mapKeyToDir: TRUE canInput: TRUE)
			)
		)
	)
			...
			(if (== (event type?) evMOUSEBUTTON)
				...
				(if
					(and ;coach, sofa
						...
						(> (event x?) 75) ;x1 (> (mouseX) (left edge of rectangle))
						(< (event x?) 160) ;x2 
						(> (event y?) 96) ;y1
						(< (event y?) 129) ;y2
					)
				...
				(if
						...
						(ClickedOnObj aSuzi (event x?) (event y?))
...
;;;											#button {land} 1 ;English
;;;											#button {decree} 2 ;English
											{&De qu+ quieres hablar?} ;Spanish
											#button {Tierras} 1 ;Spanish
											#button {Divorcio} 2 ;Spanish									
	...
	(method (changeState newState)
		...
		(switch (= state newState)
			...
			(3
				...
				(Print @str
					#at -1 10
;;;					#title {Suzi says} ;English
					#title {Suzi dice} ;Spanish
					#mode teJustCenter
					#icon 324 5 0
				)
