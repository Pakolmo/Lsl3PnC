...
;;;				(DrawRect 227 258 58 85) ;rock
;;;				(DrawRect 135 250 69 117 10) ;rope if crossable 
;;;				(DrawRect 38 237 166 189) ;marijuana
;;;			(DrawRect 68 153 21 124) ;palm
;;;				(DrawRect 1 319 21 62) ;background
;;;				;(Bset fDrankRiverWater) ;debug only 
			)
		)
	)
)
...
(instance RoomScript of Script
	...
	(method (handleEvent event &tmp temp3)
		(if (event claimed?)
		...
		(cond 
			...
			( ;left clicks
				...
				(if (== theCursor 997) ;wait cursor
					(event claimed: TRUE)
				)
				(if (== theCursor 992) ;wait cursor
					(event claimed: TRUE)
				)
    			(if (== currentStatus 536) ;top of tree?
    				(event claimed: TRUE)
        			(if (== theCursor 992) ;use wait sierra cursor to stand up.
        				(event claimed: TRUE)
            			(= temp0
							(Print
;;;								{What do you want to do now?} ;english
;;;								#button {Climb down} 1 ;english
;;;								#button {Take Coconut} 2 ;english
       			...
       			(if (ClickedInRect 38 237 166 189 event) ;plants
					(switch theCursor
					(992 ;wait cursor
					(event claimed: TRUE)
						)
						(998 ;look marijuana
							(event claimed: TRUE)	
							(Print {Ves plantas de marihuana}) ;Spanish
;;;							(Print {You look marijuana plants}) ;English
						)					
						(995 ; get plants marijuana	
							(event claimed: TRUE)	
							(if (!= currentStatus egoNORMAL)
								(GoodIdea)
							else
				...
				(if (ClickedOnObj ego (event x?) (event y?))  ;clicked patti
					(if (== theCursor iMarijuana)
						(event claimed: TRUE)
						(= temp3
							(Print
;;;								{What do you want to do?} ;english
;;;								#button {Smoke} 1 ;english
;;;								#button {A rope} 2 ;english
;;;								#button {Eat} 3 ;english
												
								{&Qu+ quieres hacer?}
								#button {Fumar} 1 ;Spanish
								#button {Una cuerda} 2 ;Spanish
								#button {Comer Hierba} 3 ;Spanish
							)
						)
						(switch temp3
							(1 ;Fumar
					...
					(if 
						(or 
							(== theCursor 17)
							(== theCursor 994)
							(== theCursor 995) ;hand on patti, rip dress
						)
						(event claimed: TRUE)
						(cond 
							((!= currentStatus egoNORMAL)
				...
				(if (ClickedInRect 135 250 69 117 event) ;rope
					(if (== ropeState 4) ;only if rope can be crossed
						(if (== theCursor 992)
							(event claimed: TRUE)
						)
						(if (== theCursor 995) ;hand
							(event claimed: TRUE)
							(cond 
								((!= currentStatus egoNORMAL)
									(Print 530 19)
				...
				(if (ClickedInRect 227 258 58 85 event) ;rock
					(event claimed: TRUE)
					(switch theCursor
						(992
							(event claimed: TRUE)
				...
				(if (ClickedInRect 68 153 21 124 event) ;palm
					(event claimed: TRUE)
					(switch theCursor
						(992 ; wait cursor
							(event claimed: TRUE)
				...
				(if (ClickedInRect 1 319 21 62 event) ;background should be last! 
					(event claimed: TRUE)
					(switch theCursor	
						(992 
						(event claimed: TRUE)				
...
(instance atpTits of PicView ;climbing tree
	(properties
		y 157
		x 169
		view 532
