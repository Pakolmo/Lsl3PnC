...
;;;	)
	...
	(method (doit)
		...
		;unless a room change is in progress
	...
	(method (handleEvent event)
		...
		(or
			(if (not (and useSortedFeatures (== (event type:) saidEvent)))
				(or
...
(procedure (PromptForDiskChange saveDisk
	...
	(if
		...
		(Format @str GAME 6
;;;			(if saveDisk {SAVE GAME} else {GAME}) ;english
			(if saveDisk {SAVE GAME} else {GAME}) ;spanish
			@curDevice
		)
		...
		(= ret
			(if saveDisk
				(Print @str
					#font: SYSFONT
;;;					#button: {OK} TRUE ;ENGLISH
;;;					#button: {Cancel} FALSE ;ENGLIH
;;;					#button: {Change Directory} 2 ;ENGLISH
					#button: {OK} TRUE ;Spanish
					#button: {Cancelar} FALSE ;Spanish
					#button: {Cambiar Directorio} 2 ;Spanish
				)
			else
				(Print @str
					#font: SYSFONT
