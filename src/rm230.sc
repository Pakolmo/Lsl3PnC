...
(procedure (DoormanSays theView theLoop theCel)
	(Print @str
		#at -1 10
;;;		#title {the Doorman says:} ;ENGLISH
		#title {El portero dice:} ;SPANISH
		#mode teJustCenter
...
(instance RoomScript of Script
	...
	(method (handleEvent event &tmp i) ;add
	;(method (handleEvent event)
		...
		(cond 
			...
			(if
				(and ;hierva,grass
						(> (event x?) 164) ;x1 (> (mouseX) (left edge of rectangle))
						(< (event x?) 248) ;x2 
						(> (event y?) 95) ;y1
						(< (event y?) 128) ;y2
					)
			...
			(if
				(and ;exit room down
						(> (event x?) 90) ;x1 (> (mouseX) (left edge of rectangle))
						(< (event x?) 230) ;x2 
						(> (event y?) 174) ;y1
						(< (event y?) 189) ;y2
					)
					...
					(if
				(and ;exit room right derecha
						(> (event x?) 306) ;x1 (> (mouseX) (left edge of rectangle))
						(< (event x?) 319) ;x2 
						(> (event y?) 96) ;y1
						(< (event y?) 162) ;y2
					)
					...
					(if							
						(and ;exit room left cave 94 118 69 133
						(> (event x?) 94) ;x1 (> (mouseX) (left edge of rectangle))
						(< (event x?) 118) ;x2 
						(> (event y?) 69) ;y1
						(< (event y?) 133) ;y2
					)
				...
				(if							
					(and
						(ClickedOnObj aDoorman (event x?) (event y?))
