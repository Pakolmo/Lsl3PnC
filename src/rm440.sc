...
;;;	(Print &rest #at 20 -1 #title {Cherri says} #width 150) ;English
	(Print &rest #at 20 -1 #title {Cherri dice} #width 150) ;Spanish
...
;;;	(Print &rest #at 180 -1 #title {You say} #width 120)
	(Print &rest #at 180 -1 #title {Dices} #width 120)
	(systemWindow color: vBLACK back: vGREY)
)
...
(instance RoomScript of Script
	...
	(method (handleEvent event)
		...
		(cond 
			...
			(if
				(and ;clothes larry
						(> (event x?) 140) ;x1 (> (mouseX) (left edge of rectangle))
						(< (event x?) 160) ;x2 
						(> (event y?) 144) ;y1
						(< (event y?) 150) ;y2
					)
