...
(procedure (StartExercising activity)
	...
	(switch (++ exerciseTimes)
		(1
			;(Print 380 30)
			(Print {Alterna con click izquierdo y derecho para hacer los ejercicios.})
;;;			(Print {Toggle between left and right click to do the exercises.}) ;English
		)
		(2
			(Print 380 31)
		)
...
(procedure (StopExercising)
	(= currentActivity 0)
	(= local12 0)
;	(Bset fNotShower) ;fix dead-ends
;	(Bset fNotUseSoap) ;fix dead-ends
;	(Bset fNotUseDeodorant) ;fix dead-ends
...
;		(if debugging (= requiredNum 5))
		(= requiredNum 10) ;EO: made this a flat 25 to fix an original speed bug
...
(instance RoomScript of Script
	...
	(method (changeState newState)
		...
		(switch (= state newState)
			...
			(4
				...
				(if local2
					(if (== (++ requiredLegCurls) legCurls)
						...
						(RoomScript changeState: 7) ;auto stop exercise
					)
			...
			(14
				...
				(if local2
					(if (== (++ requiredPoundsPumped) poundsPumped)
						(Printf 380 18 (* 100 requiredPoundsPumped)) ;auto stop exercise
			...
			(19
				(theGame setCursor: 998 (HaveMouse)) ;add
				(ego
					illegalBits: 0
					viewer: 0
					setPri: 13
...
;;;						(RoomScript changeState: 27) ;auto stop exercise
						(RoomScript changeState: 27) ;auto stop exercise
					)
					(DisplayTotalExercise)
				)
			...
			(32
				...
				(if local2
					(if (== (++ requiredBarPulls) barPulls)
						...
						(RoomScript changeState: 35) ;auto stop exercise
					)
					(DisplayTotalExercise)
				)
			)
...
(		
			...
			(if
					...
					(and ;machine left (DrawRect 106 128 146 172 10)
						
						(> (event x?) 106) ;x1 (> (mouseX) (left edge of rectangle))
						(< (event x?) 128) ;x2 
						(> (event y?) 146) ;y1
						(< (event y?) 172) ;y2
					)
		...
		(if
					...
					(and ;machine 2 (DrawRect 142 166 150 172 11)
						
						(> (event x?) 142) ;x1 (> (mouseX) (left edge of rectangle))
						(< (event x?) 166) ;x2 
						(> (event y?) 150) ;y1
						(< (event y?) 172) ;y2
					)
		...
		(if
				(and ;machine 3 (DrawRect 167 202 131 168 5)
						
						(> (event x?) 167) ;x1 (> (mouseX) (left edge of rectangle))
						(< (event x?) 202) ;x2 
						(> (event y?) 131) ;y1
						(< (event y?) 168) ;y2
					)
	...
	(if
					(and ;machine 4 (DrawRect 203 230 150 170 2)
						
						(> (event x?) 203) ;x1 (> (mouseX) (left edge of rectangle))
						(< (event x?) 230) ;x2 
						(> (event y?) 150) ;y1
						(< (event y?) 170) ;y2
					)
