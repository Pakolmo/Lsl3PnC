	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(9
				...
				(User canInput: 1) ;fix sierra cursor.
...
;;;				
			...
			(if
					(and ;cage(DrawRect 110 215 55 112 10)
...
;;;							(if ;check if clicked on aDoctor
;;;					(and
;;;						(ClickedOnObj aDoctor (event x?) (event y?))
;;;						(cast contains: aDoctor)
;;;					)
;;;					
;;;					(event claimed: TRUE)
;;;					(switch theCursor
...
;;;				)
;;;			
;;;				(if ;check if clicked on aLarry
;;;					
;;;						(ClickedOnObj aLarry (event x?) (event y?))
;;;					
;;;					
;;;					
;;;					(event claimed: TRUE)
