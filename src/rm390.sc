...
(procedure (BambiSays)
	...
	(Print @str
		#at 10 10
;;;		#title {Bambi says...} ;English
		#title {Bambi dice...} ;Spanish
		#width 140
		#mode teJustCenter
		#dispose
	)
...
;;;									{A beautiful blond woman stands on the stage with a puzzled expression on her face.} ;English
									{Una hermosa mujer rubia se encuentra en el escenario con una expresi>n de desconcierto en su rostro.} ;Spanish
								)
								((Btst fBambiGone)
;;;									{It looks like someone was recently here making a video.} ;English
									{Parece que alguien estuvo aqu- recientemente haciendo un video.} ;Spanish
								)
								(else
;;;									{The equipment seems to have suffered some sort of meltdown!} ;English
									{Parece que el equipo ha sufrido alg{n tipo de aver|a.} ;Spanish
								)
							)
						)
					)
			...
			(if ;check if clicked on Bambi
				(and
				(ClickedOnObj aBambi (event x?) (event y?))
				(cast contains: aBambi)
				)
				(event claimed: TRUE)
				(switch theCursor
					...
					(else ; 
						...
						(if
				(and ;exit room
						(> (event x?) 232) ;x1 (> (mouseX) (left edge of rectangle))
						(< (event x?) 251) ;x2 
						(> (event y?) 70) ;y1
						(< (event y?) 137) ;y2
					)
...
(instance aMike of Prop
	...
	(method (init)
		(super init:)
		(self setPri: 10 ignoreActors:) ;fix
	)
)
