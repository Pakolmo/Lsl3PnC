...
;;;		
			...
			(if
				(and ;carpet bear
						(> (event x?) 99) ;x1 (> (mouseX) (left edge of rectangle))
						(< (event x?) 187) ;x2 
						(> (event y?) 130) ;y1
						(< (event y?) 153) ;y2
				)
					(if (== theCursor 998) ;look cursor
						(Print 350 17)
						(Print 350 18
							#at -1 144
						)
			...
			(if
				...
				(if	
					(and ;aDoor
						(> (event x?) 27) ;x1 (> (mouseX) (left edge of rectangle))
						(< (event x?) 41) ;x2 
						(> (event y?) 83) ;y1
						(< (event y?) 131) ;y2
					)
					...
					(if	
						(ClickedOnObj aDave (event x?) (event y?))
				...
				(if	(ClickedOnObj aWaterfall (event x?) (event y?))
