...
(procedure (PattiSays &tmp seconds)
	(Print @str
		#at 10 10
;;;		#title {Patti says...} ;english
		#title {Patti dice...} ;spanish
		#width 140
		#mode teJustCenter
			...
			((Said 'look>')
				...
				(if ;check if clicked on aPanties
					...
					(and ;biombo
						...
						(> (event x?) 4) ;x1 (> (mouseX) (left edge of rectangle))
						(< (event x?) 58) ;x2 
						(> (event y?) 98) ;y1
						(< (event y?) 155) ;y2
					)
					...
					(and ;Table
						...
						(> (event x?) 150) ;x1 (> (mouseX) (left edge of rectangle))
						(< (event x?) 177) ;x2 
						(> (event y?) 112) ;y1
						(< (event y?) 134) ;y2
					)
